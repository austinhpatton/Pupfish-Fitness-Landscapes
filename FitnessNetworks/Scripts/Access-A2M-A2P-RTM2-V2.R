...
# THIS IS A RE-ANALYSES, USING ONLY SAMPLES FROM RTM2
...
setwd('~/Dropbox/Research/Martin-Berkeley-Postdoc/Pupfish-Fitness-Genomics/Results/FixedSweepingRTM2/')
...
#Identify which samples are from RTM1
rtm1 <- meta$ID[which(meta$Experiment == "RTM1")]

dat <- read.table('../Hybrid-GamDat.tsv', header = T)
dat <- merge(meta, dat[,-c(2,5,7:8)], by = 'ID', all = T)
dat <- dat[-which(dat$ID %in% rtm1),]
base.dir <- '~/Dropbox/Research/Martin-Berkeley-Postdoc/Pupfish-Fitness-Genomics/Results/FixedSweepingRTM2/'
...
write.table(genos, 'sgv-denovo-intro-sites-P.012-mat-RTM2.txt', sep = '\t')
...
write.table(perms, paste0(base.dir, 'sgv-denovo-intro-permuations-P-RTM2.txt'), row.names = F, quote = F, col.names = F)
...
write.table(genos, 'sgv-intro-sites-M.012-mat-RTM2.txt', sep = '\t')
genos <- merge(dat, genos, by = 'ID', all = T)
# Now, we are going to generate equal numbers of random 
# permutations that have some number of snps sourced from sgv, dnv, or intro
# and so on. Then, randomly sample 1000 from these without replacement
...
perms <- unique(perms)
perms <- perms[sample(1:12000, size = 5000, replace = F),]
write.table(perms, paste0(base.dir, 'sgv-intro-permuations-M-RTM2.txt'), row.names = F, quote = F, col.names = F)
...
plotdir <- '~/Dropbox/Research/Martin-Berkeley-Postdoc/Pupfish-Fitness-Genomics/Results/FixedSweepingRTM2/NetworkSummaries-raw/'
...
for(fitness in c('Survival', 'Growth', 'GrowthSurvival')){
  setwd(paste0('~/Dropbox/Research/Martin-Berkeley-Postdoc/Pupfish-Fitness-Genomics/Results/FixedSweepingRTM2/NetworkSummaries-raw/', fitness))
  if(fitness == 'Survival'){
    fit <- 'survival'
  }
  if(fitness == 'Growth'){
  ...
  write.table(Results, file = paste0('../', fitness, 'AllSites-Accessibility-MedMeanSE-RTM2.txt'), sep = '\t', quote = F)
  ...
  write.table(net.path.scaling.mods, col.names = T, row.names = F, quote = F, sep = "\t",
              file = paste(fitness, '-NetSize-NumPathScaling-models-RTM2.txt'))
  
  leg <- 
    get_legend(ggplot(data = meds, aes(y = NumPaths, x = NumNodes.InNet, 
                                       color = Trajectory)) +
                 stat_smooth(method = 'glm', method.args = list(family = "poisson"), 
                             se = F, aes(color = Trajectory), size = 2) + 
                 scale_color_manual(values = c(m.col, p.col), 
  ...
  ggsave(path.net.size.p, filename = paste0(fitness, '-NetSize-NumAccessiblePaths-Scaling-RTM2.pdf'), 
         height = 12, width = 12)
  ...
  ggsave(allsites.access, filename = 'AllSites-Accessibility-A2M-A2P-RTM2.pdf', width = 8, height = 8, useDingbats = F)
  ...
  ggsave(MeanFit.Len, filename = 'AllSites-MeanFit-LengthScaling-A2M-A2P-RTM2.pdf', width = 12, height = 12, useDingbats = F)
  ...
  # we also need to combine the number of nodes in each network so that we can scale the number of peaks, and number of paths 
  # by network size similar to what we did with the number of accessible paths. 
  ...
  Results[1:2,-c(1:2)] <- rbind(m.all, p.all)
  write.table(Results, file = paste0('../', fitness, 'AllSites-NetworkSummaries-MeanSE-RTM2.txt'), sep = '\t', quote = F)
  ...
  main.or <- rbind(RelNumPaths.or, MinDist.or, NumPeaks.or, MinDistToPeak.or)
  supp.or <- rbind(MeanPeakFit.or, MeanPeakSlope.or, PeakDegree.or)
  
  # Rename accessibility measures for plotting
  main.or$Accessibility <- c('Scaled # accessible paths', 'Length of shortest accessible path',
                             'Number of peaks', 'Minimum distance to peak')
  ...
  main.res.upper <- 
    plot_grid(RelNumPaths.p, min.dist.p, NumPeaks.p, 
  ...
  ggsave(main.res.p, filename = 'AllSites-A2M-A2P-AccessibilityPeakSummary-RTM2.pdf', width = 12, height = 7, useDingbats = F)
  ggsave(main.res.p, filename = 'AllSites-A2M-A2P-AccessibilityPeakSummary-RTM2.png', width = 12, height = 7)
  ...
  ggsave(supp.res.p, filename = 'AllSites-A2M-A2P-AccessibilityPeakSummary-Supps-RTM2.pdf', width = 12, height = 7, useDingbats = F)
  ggsave(supp.res.p, filename = 'AllSites-A2M-A2P-AccessibilityPeakSummary-Supps-RTM2.png', width = 12, height = 7)
  
  ggsave(NumPeaks.p, filename = 'AllSites-NumberOfPeaks-A2M-A2P-RTM2.pdf', width = 6, height = 6, useDingbats = F)
  ggsave(mean.peaks.p, filename = 'AllSites-MeanPeakSummary-A2M-A2P-RTM2.pdf', width = 12, height = 12, useDingbats = F)
  ...
  write.table(or.res, file = paste0(plotdir, fitness, '-BroadAccess-A2M-A2P-MainRes-OddsRatio-Vals-RTM2.txt'), 
