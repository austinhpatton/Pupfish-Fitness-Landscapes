# This is a slightly modified version of the vis.gam function from the mgcv R package. 
# I've modified it to allow for a little more flexibility, allowing me to provide a dataframe that includes either LD or PC scores, 
# and to allow for some alternative color plotting schemes. 

viz.gam <- 
  function (x, view = NULL, cond = list(), n.grid = 30, too.far = 0, col.limits = NULL,
            col = NA, color = "heat", contour.col = NULL, se = -1, type = "link", 
            plot.type = "persp", zlim = NULL, nCol = 50, response = "Fitness", ...) 
  {
    ...
    if (se <= 0) {
      ...
      # Similar to the above, but for color limits
      if (!is.null(col.limits)) {
        if (length(col.limits) != 2 || col.limits[1] >= col.limits[2]) 
          stop("Something wrong with col.limits")
        min.col <- col.limits[1]
        max.col <- col.limits[2]
      }
      else {
        min.col <- min(fv$fit, na.rm = TRUE)
        max.col <- max(fv$fit, na.rm = TRUE)
      }
      surf.col <- surf.col - min.col
      surf.col <- surf.col/(max.col - min.col)
      ...
      if (color == "heat") {
        ...
        cols <- scico_palette_data('davos')
        pal <- rgb(cols[,3], cols[,4], cols[,5], maxColorValue=1)
        davos.colors <- colorRampPalette(pal)
        
        pal <- davos.colors(nCol)
        con.col <- 1
      }
      else if (color == "grayC") {
        cols <- scico_palette_data('grayC')
        pal <- rgb(cols[,3], cols[,4], cols[,5], maxColorValue=1)
        grayC.colors <- colorRampPalette(pal)
        
        pal <- rev(grayC.colors(nCol*2))
        con.col <- 1
      }
    ...
    if(plot.type == "contour"){
      ...
      colnames(res) <- c('LD1', 'LD2', response)
      res$LD1 <- as.numeric(as.character(res$LD1))
      res$LD2 <- as.numeric(as.character(res$LD2))
    }
    return(res)
  }
