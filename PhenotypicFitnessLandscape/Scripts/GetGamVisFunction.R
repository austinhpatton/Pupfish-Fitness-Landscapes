get.gam.viz <- 
  function (x, cond = list(), n.grid = 60, too.far = 0.0, response = 'SurvivalProbability',
            col = NA, color = "heat", contour.col = NULL, se = -1, 
            zlim = NULL, nCol = 100, view= c('LD1', 'LD2', 'PC2', 'PC3', 'PC4'), 
            plot.type = 'contour', type = 'response', plot = T, ...) 
{
  ...
  # res <- z
  # colnames(res) <- as.numeric(v2)
  # rownames(res) <- as.numeric(v1)
  # melt(res)
  # return(list('Fitted' = data.frame(z), 'LD2' = v1, 'LD1' = v2))
  res <- as.table(z)
  colnames(res) <- m2
  rownames(res) <- m1
...
viz.gam <- 
  ...
  {
    # Set the color pallette for consistent plotting
